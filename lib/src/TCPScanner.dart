...
class TCPScanner {
  ...
  TCPScanner.range(String host, int startPort, int endPort,
      ...
      : this.build(
            host,
            List.generate(max(startPort, endPort) + 1 - min(startPort, endPort),
                (i) => min(startPort, endPort) + i),
            timeout,
            shuffle,
            isolates,
            updateInterval);
  ...
  TCPScanner.build(String host, List<int> ports, int timeout, bool shuffle,
      int isolates, Duration updateInterval) {
    _host = host;
    _ports = ports;
  ...
  Future<ScanResult> scan() async {
    ...
    while (startIndex < ports.length) {
      endIndex = startIndex + portsPerIsolate > ports.length
          ? ports.length
          : startIndex + portsPerIsolate;
    ...
    _isolateScanResults = [];
    _scanResult =
        ScanResult(host: _host, ports: _ports, status: ScanStatuses.scanning);
    // Run isolates and create listeners
    var completers = <Completer>[];
    for (var portsList in isolatePorts) {
      var completer = Completer();
      var receivePort = ReceivePort();
      var isolateScanResult = ScanResult(
          host: _host, ports: portsList, status: ScanStatuses.scanning);
      completers.add(completer);
      _isolateScanResults.add(isolateScanResult);
      await Isolate.spawn(
          _isolateScan,
          IsolateArguments(
              receivePort.sendPort, _host, portsList, _connectTimeout,
  ...
  Future<ScanResult> _noIsolateScan() async {
    ...
    for (var port in _ports) {
      try {
        connection =
            await Socket.connect(_host, port, timeout: _connectTimeout);
  ...
  static void _isolateScan(IsolateArguments arguments) async {
    ...
    for (int port in arguments.ports) {
      if (port <= 0 || port > 65536) {
        ...
        try {
          connection = await Socket.connect(arguments.host, port,
